...
// generischer Text-Reader für beliebige Property-Typen
...
// Artist über Wix-ID finden (mehrere Feldnamen/Typen)
...
// Booking-DB Struktur bestimmen + mögliche Event-Relationen sammeln
async function getBookingInfo() {
  const db = await notion.databases.retrieve({ database_id: DB_BOOK });

  // Owner (Artist) – Relation oder Rollup
  ...
  // Kandidaten für Event-Relation: Name enthält "event" oder "gig", Relation, nicht Owner
  const eventRelationCandidates = Object.entries(db.properties || {})
    .filter(([name, def]) =>
      def.type === "relation" && name !== ownerName && (/event|gig/i.test(name))
    )
    .map(([name]) => name);
...
// Für eine Booking-Seite die beste Event-Relation bestimmen
function pickEventRelationName(page, ownerName, candidates) {
  ...
  // Heuristik: irgendeine Relation ≠ Owner, die Werte hat
  for (const [name, prop] of Object.entries(page.properties || {})) {
    if (name === ownerName) continue;
    if (prop?.type === "relation" && Array.isArray(prop.relation) && prop.relation.length) {
      return name;
    }
  }
  return null;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 5) Mapping – Summary direkt aus Event-Seite lesen
    const results = [];
    for (const page of (r.results || [])) {
      ...
      // Event-Relation Kandidaten
      const eventRelName =
        pickEventRelationName(page, info.ownerName, info.eventRelationCandidates);
      ...
      if (eventRelName) {
        ...
        if (rel?.type === "relation" && Array.isArray(rel.relation) && rel.relation.length) {
          const evtId = rel.relation[0]?.id; // erste Event-Seite
          if (evtId) {
            try {
              const evtPage = await getPage(evtId);
              // **Summary direkt aus Events-DB**
              summary = readAny(evtPage, ["Summary", "Event Summary", "Zusammenfassung"]);
              if (!summary) {
                // Fallback: eigene Felder in Event (falls du statt Formel Bausteine hältst)
            ...
            } catch { /* fällt unten um */ }
          }
        }
      }

      // Letzter Fallback: Booking-Seite
      if (!summary) {
        summary = readAny(page, ["Summary"]);
        if (summary) summaryVia = "booking:Summary";
      }
      if (!summary) {
        // ganz grober Fallback (immer was anzeigen)
    ...
    if (String(debug) === "1") {
      payload.debug = {
        ...
        eventRelationCandidates: info.eventRelationCandidates
      };
    }
    res.json(payload);
  } catch (e) {
