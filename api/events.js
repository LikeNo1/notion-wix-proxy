// /api/events.js — Property-Typen erkennen & passende Filter verwenden
...
// ---- Artists-DB: Artist via WixOwnerID / Wix Owner ID / Wix Member ID finden ----
...
// ---- Booking-DB: Schema abrufen & relevante Property-Typen bestimmen ----
async function getBookingSchemaInfo() {
  const db = await notion.databases.retrieve({ database_id: DB_BOOK });

  // Helper: Kandidaten-Namen durchgehen
  const pickProp = (names) => {
    for (const n of names) {
      if (db.properties?.[n]) return { name: n, type: db.properties[n].type };
    }
    return { name: null, type: null };
  };

  const owner = pickProp(["OwnerID", "Owner ID"]);           // relation ODER rollup
  const status = pickProp(["Status"]);                       // status ODER select
  const availability = pickProp(["Artist availability"]);    // select / rich_text / formula
  const summary = pickProp(["Summary"]);                     // formula / rich_text / title

  return { owner, status, availability, summary, all: Object.keys(db.properties || {}) };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 3) Ownership-Filter passend zum Property-Typ
    ...
    // 4) Basisfilter (Potential raus – Rücksicht auf Status-Typ)
    const andFilters = [];
    if (info.status.type === "status") {
      ...
      // Falls Status gar kein Status/Select ist, lassen wir Potential-Filter weg
    }

    // 5) optional: Status-Filter (nur wenn nicht "all" und Typ bekannt)
    ...
    // 6) optional: Availability-Filter typgenau
    ...
    if (availNorm && availNorm !== "all") {
      ...
      if (availName && info.availability.name) {
