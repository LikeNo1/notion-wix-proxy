    ...
    for (const variant of [
      ...
      try {
        const r = await notion.databases.query({
          database_id: DB_ART, page_size: 1, filter: { property: p, ...variant }
        });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Owner-Property finden (Relation/Rollup)
    ...
    const andFilters = [];
    // Archiv/Potential ausblenden (serverseitig)
    const hideVals = ["Potential","Archiv","Archive"];
    if (statusProp?.type === "status") hideVals.forEach(v => andFilters.push({ property: "Status", status: { does_not_equal: v } }));
    else if (statusProp?.type === "select") hideVals.forEach(v => andFilters.push({ property: "Status", select: { does_not_equal: v } }));
    ...
    for (const page of (r.results || [])) {
      ...
      const availabilityTxt = textFrom(P(page, "Artist availability")) || textFrom(P(page, "Availability artist"));
      const comment = textFrom(P(page, "Artist comment"));
      const joyComment = textFrom(P(page, "Joy comment"));

      // Summary direkt aus Rollup/Formel
